...
if ( ! function_exists( 'organio_setup' ) ) :
	/**
	 ...
	 */
	function organio_setup() {
		// Make theme available for translation.
		load_theme_textdomain( 'organio', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'primary' => esc_html__( 'Primary', 'organio' ),
			'secondary' => esc_html__( 'Secondary', 'organio' ),
			'menu-left' => esc_html__( 'Menu Left', 'organio' ),
			'menu-right' => esc_html__( 'Menu Right', 'organio' ),
			'menu-topbar' => esc_html__( 'Menu Topbar Header Layout 2', 'organio' ),
			'menu-topbar2' => esc_html__( 'Menu Topbar Header Layout 3', 'organio' ),
			'menu-shop' => esc_html__( 'Menu Shop', 'organio' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'organio_custom_background_args', array(
        ...
        // Enable support for Post Thumbnails on posts and pages.
        add_theme_support('post-thumbnails');
        add_image_size( 'organio-thumbnail', 58, 68, true );
        add_image_size( 'organio-post', 870, 315, true );
...
add_action( 'after_setup_theme', 'organio_setup' );
 ...
 *
 */
function organio_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'organio_content_width', 640 );
}

add_action( 'after_setup_theme', 'organio_content_width', 0 );

/**
 * Register widget area.
 */
function organio_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Blog Sidebar', 'organio' ),
		'id'            => 'sidebar-blog',
		'description'   => esc_html__( 'Add widgets here.', 'organio' ),
	...
	if (class_exists('ReduxFramework')) {
		register_sidebar( array(
			'name'          => esc_html__( 'Page Sidebar', 'organio' ),
			'id'            => 'sidebar-page',
			'description'   => esc_html__( 'Add widgets here.', 'organio' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-content">',
			'after_widget'  => '</div></section>',
	...
	if ( class_exists( 'Woocommerce' ) ) {
		register_sidebar( array(
			'name'          => esc_html__( 'Shop Sidebar', 'organio' ),
			'id'            => 'sidebar-shop',
			'description'   => esc_html__( 'Add widgets here.', 'organio' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-content">',
			'after_widget'  => '</div></section>',
...
add_action( 'widgets_init', 'organio_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function organio_scripts() {
	...
	wp_enqueue_style( 'organio-theme', get_template_directory_uri() . '/assets/css/theme.css', array(), $theme->get( 'Version' ) );
	wp_add_inline_style( 'organio-theme', organio_inline_styles() );
	wp_enqueue_style( 'organio-style', get_stylesheet_uri() );
	wp_enqueue_style( 'organio-google-fonts', organio_fonts_url() );
	...
	wp_enqueue_script( 'organio-main', get_template_directory_uri() . '/assets/js/main.js', array( 'jquery' ), $theme->get( 'Version' ), true );
	wp_enqueue_script( 'organio-woocommerce', get_template_directory_uri() . '/woocommerce/woocommerce.js', array( 'jquery' ), $theme->get( 'Version' ), true );
...
add_action( 'wp_enqueue_scripts', 'organio_scripts' );

/* add admin styles */
function organio_admin_style() {
	$theme = wp_get_theme( get_template() );
	wp_enqueue_style( 'organio-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), $theme->get( 'Version' ) );
	wp_enqueue_style( 'font-flaticon', get_template_directory_uri() . '/assets/css/flaticon.css', array(), $theme->get( 'Version' ) );
	wp_enqueue_script( 'organio-main-admin', get_template_directory_uri() . '/assets/js/main-admin.js', array( 'jquery' ), $theme->get( 'Version' ), true );
}

add_action( 'admin_enqueue_scripts', 'organio_admin_style' );
...
if ( ! function_exists( 'organio_fonts_url' ) ) :
	/**
	 * Register Google fonts.
	 *
	 * Create your own organio_fonts_url() function to override in a child theme.
	 *
	 * @since league 1.1
	 *
	 * @return string Google fonts URL for the theme.
	 */
	function organio_fonts_url() {
		$fonts_url = '';
		$fonts     = array();
		$subsets   = 'latin,latin-ext';

		if ( 'off' !== _x( 'on', 'Poppins font: on or off', 'organio' ) ) {
			$fonts[] = 'Poppins:400,500,600,700';
		}
		
		if ( 'off' !== _x( 'on', 'Lora font: on or off', 'organio' ) ) {
			$fonts[] = 'Lora:400,500,600,700';
		}

		if ( 'off' !== _x( 'on', 'Barlow font: on or off', 'organio' ) ) {
			$fonts[] = 'Barlow:300,400,400i,500,500i,600,600i,700,700i';
		}

		if ( 'off' !== _x( 'on', 'Architects Daughter font: on or off', 'organio' ) ) {
			$fonts[] = 'Architects Daughter:400';
		}

		if ( 'off' !== _x( 'on', 'Fira Sans font: on or off', 'organio' ) ) {
			$fonts[] = 'Fira Sans:400,500,700';
		}

		if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'organio' ) ) {
			$fonts[] = 'Roboto:400,500,600,700';
		}

		if ( 'off' !== _x( 'on', 'Lexend font: on or off', 'organio' ) ) {
			$fonts[] = 'Lexend:400,500,600,700';
		}

		if ( 'off' !== _x( 'on', 'Playfair Display font: on or off', 'organio' ) )
        {
            $fonts[] = 'Playfair Display:400,400i,700,700i,800,900';
        }

        if ( 'off' !== _x( 'on', 'Abril Fatface font: on or off', 'organio' ) )
        {
            $fonts[] = 'Abril Fatface:400,400i,700,700i,800,900';
        }

        if ( 'off' !== _x( 'on', 'PT Sans font: on or off', 'organio' ) )
        {
            $fonts[] = 'PT Sans:400,400i,700,700i';
        }

        if ( 'off' !== _x( 'on', 'Nunito font: on or off', 'organio' ) )
        {
