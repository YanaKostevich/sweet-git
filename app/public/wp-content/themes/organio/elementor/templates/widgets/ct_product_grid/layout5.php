...
if(class_exists('Woocommerce')) {
    ...
    $sub_title = $widget->get_setting('sub_title');
    ...
    $style_l5 = $widget->get_setting('style_l5');
    $btn_readmore = $widget->get_setting('btn_readmore');
    ...
    $load_more = array(
        ...
        'btn_link' => $btn_link,
        'image' => $image,
        'template_type' => 'product_grid_layout5',
    );
    ?>
    <div id="<?php echo esc_attr($html_id) ?>" class="ct-grid ct-product-grid ct-product-grid-layout5 ct-product-carousel-reset1 woocommerce <?php echo esc_attr($style_l5); ?>" data-layout="<?php echo esc_attr($layout_type); ?>" data-start-page="<?php echo esc_attr($paged); ?>" data-max-pages="<?php echo esc_attr($max); ?>" data-total="<?php echo esc_attr($total); ?>" data-perpage="<?php echo esc_attr($limit); ?>" data-next-link="<?php echo esc_attr($next_link); ?>">
        ...
        <div class="<?php echo esc_attr($grid_class); ?> animate-time" data-gutter="<?php echo esc_attr($gap_item); ?>" data-class-filter="
            <?php foreach ($categories as $category): ?>
                <?php $category_arr = explode('|', $category); ?>
                <?php $tax[] = $category_arr[1]; ?>
                <?php $term = get_term_by('slug',$category_arr[0], $category_arr[1]); ?>
            ...
            <?php
            $load_more['tax'] = $tax;
            organio_get_post_grid($posts, $load_more);
            ?>
        ...
        <?php if ($layout_type == 'masonry' && $pagination_type == 'pagination') { ?>
            <div class="ct-grid-pagination" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>" data-query="<?php echo esc_attr(json_encode($args)); ?>">
                <?php organio_posts_pagination($query, true); ?>
            </div>
        <?php } ?>
        <?php if (!empty($next_link) && $layout_type == 'masonry' && $pagination_type == 'loadmore') { ?>
            <div class="ct-load-more text-center" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>">
                <span class="btn btn-animate">
                    <?php echo esc_html__('Load more', 'organio') ?>
